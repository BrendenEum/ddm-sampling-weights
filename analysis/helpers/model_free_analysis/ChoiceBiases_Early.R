...
pdata = data[data$sample_num %in% c(2,3,4), ] %>%
...
p.ChoiceBiases.EarlySampleBias = ggplot(data = pdata, aes(x=slot_mean, y=y, group=sample_bin)) +
...
p.ChoiceBiases.EarlySampleBias = p.ChoiceBiases.EarlySampleBias +
  theme(plot.background = element_rect(fill = .color_e, color = .color_e))

ggsave(file.path(.figdir, "ChoiceBiases_EarlySampleBias.pdf"), p.ChoiceBiases.EarlySampleBias, width=.figw, height=.figh)