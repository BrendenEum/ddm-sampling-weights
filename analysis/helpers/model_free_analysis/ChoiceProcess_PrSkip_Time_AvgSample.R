...
plt = ggplot(data = pdata, aes(x=time_elapsed, y=y, group=avgSampleSeen)) +
  ...
  labs(y="Pr(Skip)", x="Time Elapsed (s)", color="Sample") +
...
ggsave(file.path(.figdir, "ChoiceProcess_PrSkip_Time_AvgSample.pdf"), plt, width=.figw, height=.figh)
