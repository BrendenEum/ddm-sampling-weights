...
plt = ggplot(data = pdata, aes(x=time_elapsed, y=y, group=avgSampleSeen)) +
  ...
  labs(y="Pr(Play)", x="Time Elapsed (s)", color="Rnd. Sample") +
  coord_cartesian(xlim = c(0,12), ylim = c(0,1), expand=F) +
  .gradientOpt +
  theme(
    legend.position = c(.105, .76)
  )

plt = plt +
  theme(plot.background = element_rect(fill = .color_e, color = .color_e))

ggsave(file.path(.figdir, "ChoiceProcess_PrPlay_Time_AvgSample.pdf"), plt, width=.figw, height=.figh)
